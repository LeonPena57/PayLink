...
export function PostModal({ isOpen, onClose, post, onUpdate }: PostModalProps) {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6">
            ...
            <div className="relative w-full max-w-6xl h-[90vh] bg-background rounded-3xl overflow-hidden flex flex-col md:flex-row shadow-2xl animate-in fade-in zoom-in-95 duration-200">
                <button
                    onClick={onClose}
                ...
                {/* Sidebar Section */}
                <div className="w-full md:w-[400px] lg:w-[450px] flex flex-col bg-background border-l border-border">
                    {/* Header */}
                        ...
                        <div className="flex items-center gap-3">
                            <Link href={`/${post.user?.username || post.profiles?.username}`} className="block">
                                <div className="w-10 h-10 rounded-full bg-muted overflow-hidden border border-border">
                                    <img
                                        src={post.user?.avatar || post.profiles?.avatar_url || "https://github.com/shadcn.png"}
                                        alt="Avatar"
                            ...
                            <div>
                                <Link href={`/${post.user?.username || post.profiles?.username}`} className="font-bold hover:underline">
                    ...
                    <div className="flex-1 overflow-y-auto p-4 space-y-6">
                        ...
                        <div className="flex gap-3">
                            ...
                            <div className="flex-1 space-y-1">
                                <p className="text-sm">
                                    <span className="font-bold mr-2">{post.user?.name || post.profiles?.full_name}</span>
                        ...
                        {loadingComments ? (
                            ...
                            comments.map((comment) => (
                                <div key={comment.id} className="flex gap-3 group">
                                    <Link href={`/${comment.profiles?.username}`} className="w-8 h-8 rounded-full bg-muted overflow-hidden border border-border shrink-0">
                                        <img
                        ...
                        <div className="flex items-center justify-between mb-4">
                            <div className="flex items-center gap-4">
                                ...
                                >
                                    <Heart
                                        className={clsx(
                                            "w-7 h-7 transition-all active:scale-75",
                                            isLiked ? "fill-red-500 text-red-500" : "text-foreground group-hover:text-red-500"
                                        )}
                                    />
                                </button>
                                ...
                                <button className="group flex items-center gap-1.5 transition-colors">
                                    <Send className="w-7 h-7 text-foreground group-hover:text-green-500 transition-colors -rotate-45 translate-y-[-2px]" />
                                </button>
                            </div>
                        </div>
